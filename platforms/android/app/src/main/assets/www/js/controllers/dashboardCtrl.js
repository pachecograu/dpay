MyApp.angular.controller('dashboardCtrl', ['$scope', function ($scope) {
  ...
  $scope.updateDashboard = function () {
    try {
      ...
      $scope.db.collection("usuarios").get().then(function (querySnapshot) {
        $scope.users = [];
      ...
      $scope.db.collection("prestamos").get().then(function (querySnapshot) {
        $scope.prestamos = {
          total: 0,
          totalCartera: 0,
          data: []
      ...
      $scope.db.collection("cobros").get().then(function (querySnapshot) {
        $scope.cobros = {
          total: 0,
          data: []
        };
